...
def encode(imgd, imgf, ext='.png'):
  """
  Given a set of black white images (that probably have 3 channels), convert it
  into a gzipped file that has all the information that is in standard MNIST files
  ...
  o.write(struct.pack('>i', 2051))       # Magic Number for train/test images
  o.write(struct.pack('>i', num_imgs))   # Number of images
  ...
  for img in range(num_imgs):
    # im = cv2.imread(fs[img])[:,:,0]       # images must be one dimensional grayscale
    im = np.asarray(Image.open(fs[img]).convert('L'), dtype=np.uint32)
    for i,row in enumerate(im):
      for j,column in enumerate(im):
